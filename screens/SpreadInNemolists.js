...
const SpreadInNemolists = ({route, navigation}) => {
    // const { bookmarks, } = useSelector(bookmarkSelector);
    const dispatch = useDispatch();
    const { subTitle, title, index, albumId, } = route.params;
    const [ref, setRef] = useState(null);
    const flatListRef = useRef();
    const [pullLoading, setPullLoading] = useState(false);
    const insets = useSafeAreaInsets();
    const [orderedBookmarks, setOrderedBookmarks] = useState(route.params.bookmarks);
    ...
    return (
        <View style={styles.container}>
            ...
            {(orderedBookmarks !== null) && (orderedBookmarks !== undefined) ? 
                <FlatList 
                    ...
                    initialNumToRender={orderedBookmarks.length}
                    onScrollToIndexFailed={(error) => {
                        ...
                        let scrollTimer = setTimeout(() => {
                            // setScrollLoading(false);
                            if (orderedBookmarks.length !== 0 && flatListRef !== null) {
...
export default SpreadInNemolists;